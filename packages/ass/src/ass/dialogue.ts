import { formatColor, getDecoratingColor, isWhite } from "../util/index.ts";
import { Danmaku, DanmakuType } from "../types.ts";
...
const scrollCommand = ({ start, end, top }: any) =>
	`\\move(${start},${top},${end},${top})`;
const fixCommand = ({ top, left }: any) => `\\an8\\pos(${left},${top})`;
const colorCommand = (color: any) => `\\c${formatColor(color)}`;
const borderColorCommand = (color: any) => `\\3c${formatColor(color)}`;

export default (danmaku: Danmaku, config: any) => {
