...
import { arrayOfLength } from "./lang.ts";
...
const splitGrids = (
	{ fontSize, padding, playResY, bottomSpace }: {
		fontSize: any;
		padding: any;
		playResY: number;
		bottomSpace: number;
...
// 找到能用的行
const resolveAvailableFixGrid = (grids: any, time: any) => {
...
const resolveAvailableScrollGrid = (
	...
	for (let i = 0; i < grids.length; i++) {
		...
		const previousInTime = previous.start +
			computeScrollInTime(previous.width, screenWidth, duration);
		const currentOverTime = time +
			computeScrollOverTime(rectWidth, screenWidth, duration);
...
const initializeLayout = (config: SubtitleStyle) => {
	...
	return (danmaku: Danmaku) => {
		...
		const rectWidth =
			...
			paddingLeft +
			paddingRight;
		const verticalOffset = paddingTop +
			Math.round((defaultFontSize - danmakuFontSize) / 2);
		const gridNumber = danmaku.type === DanmakuType.SCROLL
